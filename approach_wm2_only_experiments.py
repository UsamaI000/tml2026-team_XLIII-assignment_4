#!/usr/bin/env python3
"""
WM_2-only Watermark Forgery Experiments
=======================================
...
ONLY the WM_2 target batch: 26.png ... 50.png.

Why:
    The assignment has 8 watermark families. A global recipe can hide which
    family is helping or hurting. By changing only WM_2, leaderboard movement
    tells us whether the WM_2 configuration is a bottleneck.
...
# These profiles change only WM_2. Use current-best submission ZIP as --base_zip.
# Baseline context:
#   - per_wm_consistency_v1 uses WM_2: residual_strength=4, sigma=6, f=0.75, band=0.04-0.20.
#   - global hybrid used sigma=4. These profiles test whether WM_2 prefers global, broader,
#     lower frequency, no frequency, or different aggregation.
PROFILES: Dict[str, WM2Config] = {
    # Diagnostic: revert only WM_2 to the old global-hybrid scale.
    "wm2_global_revert_sig4": WM2Config(4.0, (4.0,), 0.75, 0.04, 0.20),

    # Local strength sweep around the current per-WM setting sigma=6.
    "wm2_sig6_s35": WM2Config(3.5, (6.0,), 0.75, 0.04, 0.20),
    "wm2_sig6_s45": WM2Config(4.5, (6.0,), 0.75, 0.04, 0.20),

    # Broader residual tests. If WM_2 is more low/mid-frequency, these may help.
    "wm2_sig8": WM2Config(4.0, (8.0,), 0.75, 0.04, 0.20),
    "wm2_sig10": WM2Config(4.0, (10.0,), 0.75, 0.04, 0.20),

    # Frequency diagnostics for WM_2.
    "wm2_no_freq": WM2Config(4.0, (6.0,), 0.00, 0.04, 0.20),
    "wm2_freq_narrow": WM2Config(4.0, (6.0,), 0.75, 0.04, 0.16),
    ...
    # Channel diagnostic.
    "wm2_luma": WM2Config(4.0, (6.0,), 0.75, 0.04, 0.20, channel_mode="luma"),
}
...
def estimate_residual_fingerprint(source_paths: Sequence[Path], cfg: WM2Config) -> np.ndarray:
    residuals = []
    expected_shape = None
...
def estimate_frequency_fingerprint(source_paths: Sequence[Path], cfg: WM2Config) -> np.ndarray:
...
def build_submission(args: argparse.Namespace) -> None:
    ...
    source_dir = dataset.watermarked_sources / "WM_2"
    source_paths = sorted(source_dir.glob("*.png"), key=numeric_key)
    if len(source_paths) != 25:
        raise RuntimeError(f"Expected 25 WM_2 source images in {source_dir}, found {len(source_paths)}")
    ...
    print(
        f"WM_2 perturbation range=[{perturbation.min():.2f},{perturbation.max():.2f}], "
    ...
    if args.print_psnr:
        print(f"WM_2 mean PSNR vs clean targets: {mean_psnr_for_wm1(dataset.clean_targets, args.out_dir):.2f} dB")
    print(f"Done. Created {args.zip_out}. Only 26.png ... 50.png were regenerated.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run WM_2-only forging experiments on top of an existing base ZIP.")
    parser.add_argument("--zip_file", type=Path, default=Path("Dataset.zip"))
    parser.add_argument("--dataset_dir", type=Path, default=Path("_dataset_extracted_wm2_only"))
    parser.add_argument(
        "--base_zip",
        type=Path,
        default=Path("submission_per_wm_consistency_v1.zip"),
        help="Existing 200-image submission ZIP to keep for all non-WM2 images. Prefer current best ZIP.",
    )
    parser.add_argument("--out_dir", type=Path, default=Path("submission_temp_wm2_only"))
    parser.add_argument("--zip_out", type=Path, default=Path("submission_wm2_only.zip"))
