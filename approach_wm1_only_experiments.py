#!/usr/bin/env python3
"""
WM_1-only Watermark Forgery Experiments
=======================================

This script is for one-WM-at-a-time diagnosis. It starts from an existing
best submission ZIP, keeps images 26.png ... 200.png unchanged, and regenerates
ONLY the WM_1 target batch: 1.png ... 25.png.

Why:
    The assignment has 8 watermark families. A global recipe can hide which
    family is helping or hurting. By changing only WM_1, leaderboard movement
    tells us whether the WM_1 configuration is a bottleneck.
...
# These profiles change only WM_1. Use current-best submission ZIP as --base_zip.
# Baseline context:
#   - per_wm_consistency_v1 uses WM_1: residual_strength=4, sigma=6, f=0.75, band=0.04-0.20.
#   - global hybrid used sigma=4. These profiles test whether WM_1 prefers global, broader,
#     lower frequency, no frequency, or different aggregation.
PROFILES: Dict[str, WM1Config] = {
    # Diagnostic: revert only WM_1 to the old global-hybrid scale.
    "wm1_global_revert_sig4": WM1Config(4.0, (4.0,), 0.75, 0.04, 0.20),

    # Local strength sweep around the current per-WM setting sigma=6.
    "wm1_sig6_s35": WM1Config(3.5, (6.0,), 0.75, 0.04, 0.20),
    "wm1_sig6_s45": WM1Config(4.5, (6.0,), 0.75, 0.04, 0.20),

    # Broader residual tests. If WM_1 is more low/mid-frequency, these may help.
    "wm1_sig8": WM1Config(4.0, (8.0,), 0.75, 0.04, 0.20),
    "wm1_sig10": WM1Config(4.0, (10.0,), 0.75, 0.04, 0.20),

    # Frequency diagnostics for WM_1.
    "wm1_no_freq": WM1Config(4.0, (6.0,), 0.00, 0.04, 0.20),
    "wm1_freq_narrow": WM1Config(4.0, (6.0,), 0.75, 0.04, 0.16),
    "wm1_freq_wide": WM1Config(4.0, (6.0,), 0.75, 0.03, 0.25),
    "wm1_freq_lowmid": WM1Config(4.0, (6.0,), 0.75, 0.02, 0.12),

    # Aggregation diagnostics.
    "wm1_mean": WM1Config(4.0, (6.0,), 0.75, 0.04, 0.20, aggregation="mean"),
    "wm1_trimmed": WM1Config(4.0, (6.0,), 0.75, 0.04, 0.20, aggregation="trimmed_mean"),

    # Channel diagnostic.
    "wm1_luma": WM1Config(4.0, (6.0,), 0.75, 0.04, 0.20, channel_mode="luma"),
}
...
def estimate_residual_fingerprint(source_paths: Sequence[Path], cfg: WM1Config) -> np.ndarray:
    residuals = []
    expected_shape = None
...
def estimate_frequency_fingerprint(source_paths: Sequence[Path], cfg: WM1Config) -> np.ndarray:
...
def mean_psnr_for_wm1(clean_dir: Path, out_dir: Path) -> float:
    values = []
    for i in WM1_TARGETS:
...
def build_submission(args: argparse.Namespace) -> None:
    ...
    source_dir = dataset.watermarked_sources / "WM_1"
    source_paths = sorted(source_dir.glob("*.png"), key=numeric_key)
    if len(source_paths) != 25:
        raise RuntimeError(f"Expected 25 WM_1 source images in {source_dir}, found {len(source_paths)}")
    ...
    print(
        f"WM_1 perturbation range=[{perturbation.min():.2f},{perturbation.max():.2f}], "
    ...
    if args.print_psnr:
        print(f"WM_1 mean PSNR vs clean targets: {mean_psnr_for_wm1(dataset.clean_targets, args.out_dir):.2f} dB")
    print(f"Done. Created {args.zip_out}. Only 1.png ... 25.png were regenerated.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run WM_1-only forging experiments on top of an existing base ZIP.")
    parser.add_argument("--zip_file", type=Path, default=Path("Dataset.zip"))
    parser.add_argument("--dataset_dir", type=Path, default=Path("_dataset_extracted_wm1_only"))
    parser.add_argument(
        "--base_zip",
        type=Path,
        default=Path("submission_per_wm_consistency_v1.zip"),
        help="Existing 200-image submission ZIP to keep for all non-WM1 images. Prefer current best ZIP.",
    )
    parser.add_argument("--out_dir", type=Path, default=Path("submission_temp_wm1_only"))
    parser.add_argument("--zip_out", type=Path, default=Path("submission_wm1_only.zip"))
